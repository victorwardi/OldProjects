...
$maxRows_promo = 10;
$pageNum_promo = 0;
if (isset($_GET['pageNum_promo'])) {
  $pageNum_promo = $_GET['pageNum_promo'];
}
$startRow_promo = $pageNum_promo * $maxRows_promo;

mysql_select_db($database_CostaverdeFM, $CostaverdeFM);
$query_promo = "SELECT * FROM promo ORDER BY id DESC";
$query_limit_promo = sprintf("%s LIMIT %d, %d", $query_promo, $startRow_promo, $maxRows_promo);
$promo = mysql_query($query_limit_promo, $CostaverdeFM) or die(mysql_error());
$row_promo = mysql_fetch_assoc($promo);

if (isset($_GET['totalRows_promo'])) {
  $totalRows_promo = $_GET['totalRows_promo'];
} else {
  $all_promo = mysql_query($query_promo);
  $totalRows_promo = mysql_num_rows($all_promo);
}
$totalPages_promo = ceil($totalRows_promo/$maxRows_promo)-1;
          ...
          <table width="100%" border="0" cellspacing="2" cellpadding="0">
            <tr>
              <th width="73%" align="center" valign="top" scope="col"><table width="100%" border="0" cellspacing="1" cellpadding="0">
                ...
                <tr>
                  <th align="center" valign="top" scope="row"><?php do { ?>
                      <table width="95%" border="0" cellspacing="2" cellpadding="0">
                        <tr>
                          <th scope="col"><table width="95%" border="0" cellpadding="0" cellspacing="2" class="borda_pont_preta_botton">
                              <tr>
                                <th width="23%" align="left" valign="top" scope="col">&nbsp;<a href="promocao.php?id=<?php echo $row_promo['id']; ?>"><img src="<?php echo tNG_showDynamicThumbnail("", "fotos/", "{promo.imagem}", 100, 0, true); ?>" alt="<?php echo $row_promo['nome']; ?>" border="0" class="borda_preta_1px" /></a></th>
                                <th width="77%" align="left" valign="top" scope="col"><span class="top_cantor"><a href="promocao.php?id=<?php echo $row_promo['id']; ?>" class="top_num"><?php echo $row_promo['nome']; ?></a></span><br />
                                      <span class="top_musica"><?php echo $row_promo['data']; ?></span><br />
                                      <table width="100" border="0" cellpadding="0" cellspacing="2" class="borda_preta_1px" id="link">
                                        <tr>
                                          <th align="center" scope="col"><span class="top_musica"><a href="promocao.php?id=<?php echo $row_promo['id']; ?>">Saiba mais</a></span></th>
                                        </tr>
                                      </table></th>
                              </tr>
                          </table></th>
                      ...
                      <?php } while ($row_promo = mysql_fetch_assoc($promo)); ?></th>
                </tr>
              </table></th>
