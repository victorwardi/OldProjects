	...
	if(flood == 0){
		...
		for(i = 1; i < 27; i++){
			while(mensagem_rep.indexOf(':'+ i +':') != -1){
				mensagem_rep = mensagem_rep.replace(':'+ i +':','<img src="smiles/'+ A?o???i +'.gif">');
			}
		}
		//envia mensagem
		...
		document.getElementById("exibefrase").innerHTML  = 'O sistema anti-flood está ativado.';
	}
}
...
function Ler(){	
	obj_mostra.open("POST","ler.php",true);
...
function aba(id){
	...
	//document.getA?o?ƒ????ElementById('int3').style.backgroundColor = '#FFFFFF';
...
function intsmiles(){
	...
	exibe += '</tr><tr> ';
	exibe += '<td><a href="#" onClick="enviasmile(10)"><img src="smiles/10.giA?o???ø??????f" width="20" height="20"  border="0"></a></td>';
	...
	exibe += '<td><a href="#" onClick="enviasmile(26)"><img src="smiles/26.gif" widA?o???ø??????th="20" height="20" border="0"></a></td>';
	exibe += '</tr></table>';
...
function intcor(){
	...
	exibe += '<td width="18" bgcolor="#666600" onClick="alteracor(4)">&nA?o???ø??????bsp;</td>';
	...
	exibe += '<td bgcolor="#FF9900" onClick=A?o???ø??????"alteracor(31)">&nbsp;</td>';
...
function frase(){
	...
	if(frase != 'Digite uma frase pessoal'){
		obj_frase = new mA?o???ø??????ontaXMLHTTP();
