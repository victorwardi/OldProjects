...
if ($static == 'static') {
		...
		$static_rater[] .= '</ul>';
		$static_rater[] .= '<p class="static">'.$id.'. Classifica&ccedil;&atilde;o: <strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.' cast) <em>This is \'static\'.</em></p>';
		$static_rater[] .= '</div>';
		$static_rater[] .= '</div>'."\n\n";
      ...
      $rater.='>'' Nota: <strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.' )';
      $rater.='  </p>';
      $rater.='</div>';
      $rater.='</div>';
      return $rater;
