<?php
require_once(dirname(__FILE__) . "/Shopconn.php");
...
function findAll() {
	 ...
	 */
	$fields = array('id','titulo','materia','data','categoria');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `novidades` $where"); 
	...
	$query_recordset = "SELECT id,titulo,materia,data,categoria FROM `novidades` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `novidades` (titulo,materia,data,categoria) VALUES (%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["titulo"], "text"), # 
			GetSQLValueString($_REQUEST["materia"], "text"), # 
			GetSQLValueString($_REQUEST["data"], "text"), # 
			GetSQLValueString($_REQUEST["categoria"], "text")# 
	);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `novidades` WHERE id = %s",
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `novidades` SET titulo = %s,materia = %s,data = %s,categoria = %s WHERE id = %s", 
			GetSQLValueString($_REQUEST["titulo"], "text"), 
			GetSQLValueString($_REQUEST["materia"], "text"), 
			GetSQLValueString($_REQUEST["data"], "text"), 
			GetSQLValueString($_REQUEST["categoria"], "text"), 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `novidades` WHERE id = %s", 
			GetSQLValueString($row_recordset["id"], "int")
		);
