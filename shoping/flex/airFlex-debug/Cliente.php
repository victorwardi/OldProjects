<?php
require_once(dirname(__FILE__) . "/SisintConDBconn.php");
...
function findAll() {
	 ...
	 */
	$fields = array('clienteID','nome','razaoSocial','cnpj','inscricaoEstadual','tel01','tel02','fax','celular','email','responsavel','enderecoID');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `cliente` $where"); 
	...
	$query_recordset = "SELECT clienteID,nome,razaoSocial,cnpj,inscricaoEstadual,tel01,tel02,fax,celular,email,responsavel,enderecoID FROM `cliente` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `cliente` (nome,razaoSocial,cnpj,inscricaoEstadual,tel01,tel02,fax,celular,email,responsavel,enderecoID) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["nome"], "text"), # 
			GetSQLValueString($_REQUEST["razaoSocial"], "text"), # 
			GetSQLValueString($_REQUEST["cnpj"], "text"), # 
			GetSQLValueString($_REQUEST["inscricaoEstadual"], "text"), # 
			GetSQLValueString($_REQUEST["tel01"], "text"), # 
			GetSQLValueString($_REQUEST["tel02"], "text"), # 
			GetSQLValueString($_REQUEST["fax"], "text"), # 
			GetSQLValueString($_REQUEST["celular"], "text"), # 
			GetSQLValueString($_REQUEST["email"], "text"), # 
			GetSQLValueString($_REQUEST["responsavel"], "text"), # 
			GetSQLValueString($_REQUEST["enderecoID"], "int")# 
	);
		...
		$toret = array(
			"data" => array(
				array(
					...
					"tel01" => $_REQUEST["tel01"], # 
					"tel02" => $_REQUEST["tel02"], # 
					"fax" => $_REQUEST["fax"], # 
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `cliente` WHERE clienteID = %s",
		GetSQLValueString($_REQUEST["clienteID"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `cliente` SET nome = %s,razaoSocial = %s,cnpj = %s,inscricaoEstadual = %s,tel01 = %s,tel02 = %s,fax = %s,celular = %s,email = %s,responsavel = %s,enderecoID = %s WHERE clienteID = %s", 
			...
			GetSQLValueString($_REQUEST["responsavel"], "text"), 
			GetSQLValueString($_REQUEST["enderecoID"], "int"), 
			GetSQLValueString($row_recordset["clienteID"], "int")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"clienteID" => $row_recordset["clienteID"], 
						"nome" => $_REQUEST["nome"], #
						"razaoSocial" => $_REQUEST["razaoSocial"], #
						"cnpj" => $_REQUEST["cnpj"], #
						"inscricaoEstadual" => $_REQUEST["inscricaoEstadual"], #
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `cliente` WHERE clienteID = %s", 
			GetSQLValueString($row_recordset["clienteID"], "int")
		);
