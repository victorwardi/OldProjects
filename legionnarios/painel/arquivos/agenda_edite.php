...
// Filter
$tfi_listagenda1 = new TFI_TableFilter($conn_legion, "tfi_listagenda1");
...
$tfi_listagenda1->addColumn("agenda.data", "STRING_TYPE", "data", "%");
$tfi_listagenda1->addColumn("agenda.descricao", "STRING_TYPE", "descricao", "%");
$tfi_listagenda1->addColumn("agenda.foto", "FILE_TYPE", "foto", "%");
...
$tso_listagenda1->addColumn("agenda.foto");
$tso_listagenda1->setDefault("agenda.id DESC");
$tso_listagenda1->Execute();

// Navigation
$nav_listagenda1 = new NAV_Regular("nav_listagenda1", "rsagenda1", "../../", $_SERVER['PHP_SELF'], 10);
...
$query_rsagenda1 = sprintf("SELECT agenda.id, agenda.titulo, agenda.local, agenda.data, agenda.descricao, agenda.foto FROM agenda WHERE %s ORDER BY %s", $NXTFilter_rsagenda1, $NXTSort_rsagenda1);
$query_limit_rsagenda1 = sprintf("%s LIMIT %d, %d", $query_rsagenda1, $startRow_rsagenda1, $maxRows_rsagenda1);
$rsagenda1 = mysql_query($query_limit_rsagenda1, $legion) or die(mysql_error());
          ...
          <table cellpadding="2" cellspacing="0" class="KT_tngtable">
            <thead>
              <tr class="KT_row_order">
                ...
                <th id="titulo" class="KT_sorter KT_col_titulo <?php echo $tso_listagenda1->getSortIcon('agenda.titulo'); ?>"> <a href="<?php echo $tso_listagenda1->getSortLink('agenda.titulo'); ?>">Titulo</a> </th>
                <th id="foto" class="KT_sorter KT_col_foto <?php echo $tso_listagenda1->getSortIcon('agenda.foto'); ?>"> <a href="<?php echo $tso_listagenda1->getSortLink('agenda.foto'); ?>">Foto</a> </th>
                <th>&nbsp;</th>
              </tr>
              <?php 
...
?>
                <tr class="KT_row_filter">
                  <td>&nbsp;</td>
                  <td><input type="text" name="tfi_listagenda1_titulo" id="tfi_listagenda1_titulo" value="<?php echo KT_escapeAttribute(@$_SESSION['tfi_listagenda1_titulo']); ?>" size="20" maxlength="50" /></td>
                  <td><input type="text" name="tfi_listagenda1_foto" id="tfi_listagenda1_foto" value="<?php echo KT_escapeAttribute(@$_SESSION['tfi_listagenda1_foto']); ?>" size="20" maxlength="50" /></td>
              ...
              <?php if ($totalRows_rsagenda1 > 0) { // Show if recordset not empty ?>
                <?php do { ?>
                  <tr class="<?php echo @$cnt1++%2==0 ? "" : "KT_even"; ?>">
                    ...
                    <td><div class="KT_col_titulo"><?php echo KT_FormatForList($row_rsagenda1['titulo'], 20); ?></div></td>
                    <td align="center" valign="middle"><img src="<?php echo tNG_showDynamicThumbnail("../../", "../../fotos/", "{rsagenda1.foto}", 60, 42, false); ?>" /></td>
                    <td><a class="KT_edit_link" href="agenda_add.php?id=<?php echo $row_rsagenda1['id']; ?>&amp;KT_back=1"><?php echo NXT_getResource("edit_one"); ?></a> <a class="KT_delete_link" href="#delete"><?php echo NXT_getResource("delete_one"); ?></a> </td>
                  </tr>
...
<span>&nbsp;</span>
            ...
            <a class="KT_additem_op_link" href="agenda_add.php?KT_back=1" onclick="return nxt_list_additem(this)"><?php echo NXT_getResource("add new"); ?></a> </div>
        </form>
      </div>
