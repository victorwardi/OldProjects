...
$ins_local = new tNG_multipleInsert($conn_fotos);
$tNGs->addTransaction($ins_local);
// Register triggers
$ins_local->registerTrigger("STARTER", "Trigger_Default_Starter", 1, "POST", "KT_Insert1");
$ins_local->registerTrigger("BEFORE", "Trigger_Default_FormValidation", 10, $formValidation);
$ins_local->registerTrigger("END", "Trigger_Default_Redirect", 99, "../../includes/nxt/back.php");
// Add columns
$ins_local->setTable("local");
$ins_local->addColumn("nome", "STRING_TYPE", "POST", "nome");
$ins_local->setPrimaryKey("id", "NUMERIC_TYPE");

// Make an update transaction instance
$upd_local = new tNG_multipleUpdate($conn_fotos);
$tNGs->addTransaction($upd_local);
// Register triggers
$upd_local->registerTrigger("STARTER", "Trigger_Default_Starter", 1, "POST", "KT_Update1");
$upd_local->registerTrigger("BEFORE", "Trigger_Default_FormValidation", 10, $formValidation);
$upd_local->registerTrigger("END", "Trigger_Default_Redirect", 99, "../../includes/nxt/back.php");
// Add columns
$upd_local->setTable("local");
$upd_local->addColumn("nome", "STRING_TYPE", "POST", "nome");
$upd_local->setPrimaryKey("id", "NUMERIC_TYPE", "GET", "id");

// Make an instance of the transaction object
$del_local = new tNG_multipleDelete($conn_fotos);
$tNGs->addTransaction($del_local);
// Register triggers
$del_local->registerTrigger("STARTER", "Trigger_Default_Starter", 1, "POST", "KT_Delete1");
$del_local->registerTrigger("END", "Trigger_Default_Redirect", 99, "../../includes/nxt/back.php");
// Add columns
$del_local->setTable("local");
$del_local->setPrimaryKey("id", "NUMERIC_TYPE", "GET", "id");

// Execute all the registered transactions
$tNGs->executeTransactions();

// Get the transaction recordset
$rslocal = $tNGs->getRecordset("local");
...
?>
                        <table cellpadding="2" cellspacing="0" class="KT_tngtable">
                          <tr>
                            ...
                            <td><input type="text" name="nome_<?php echo $cnt1; ?>" id="nome_<?php echo $cnt1; ?>" value="<?php echo KT_escapeAttribute($row_rslocal['nome']); ?>" size="40" maxlength="100" />
                                <?php echo $tNGs->displayFieldHint("nome");?> <?php echo $tNGs->displayFieldError("local", "nome", $cnt1); ?> </td>
                          </tr>
                        </table>
                        <input type="hidden" name="kt_pk_local_<?php echo $cnt1; ?>" class="id_field" value="<?php echo KT_escapeAttribute($row_rslocal['kt_pk_local']); ?>" />
                        <?php } while ($row_rslocal = mysql_fetch_assoc($rslocal)); ?>
